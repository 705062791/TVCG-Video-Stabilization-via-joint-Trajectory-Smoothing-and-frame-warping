...
void union_smooth::solve_by_Eigen()
{
	//首先建立优化方程
	//总共有5项 
	//项                path_data           path_smooth            mesh_data        mesh_smooth              mesh_time_smooth
	//参数              所有特征点          所有特征点             所有特征点       网格顶点数*帧数          网格顶点数*帧数
	...
	writr_computered_path("D:/data/dst_path.txt", dst_path);
	...
	//int count = 0;
	//for (int i = num_of_feature_points*2; i <num_of_feature_points * 2+mesh_height*mesh_width; i++)
	//{
	//	if (count == mesh_width)
	//	{
	//		cout << endl << endl;
	//		count = 0;
	//	}
	//	cout << "(" << x_sparse[i] << "," << x_sparse[i + frame_num*mesh_height*mesh_width] << ")"<<"  ";
	//	count++;
	//}
...
void union_smooth::Creat_path_smooth_term2(ArrayDou& B)
{
	int term_count = 0;
	int col_count = 0;
	...
	for (int i = 0; i < org_paths.size(); i++)
	{
		for (int j = 0; j < org_paths[i].PointPath.size(); j++)
		{
			...
			Path_smooth_term2[term_count][2] = 1 * b;
			term_count++;
	...
	for (int t = 0; t < frame_num; t++)
	{
		for (int i = 0; i < mesh_height - 1; i++)
		{
			for (int j = 0; j < mesh_width - 1; j++)
			{
				...
				//homos_avg.at(i, j) = cv::findHomography(s_, d_);
...
void union_smooth::warping_video(double fps)
{
	...
	for (int t = 0; t < frame_num - 1; t++)
	{
		...
		Mat org;
		Mat biger_org(h, w, CV_8UC3, cv::Scalar(255, 255, 255));
		Mat dst(h, w, CV_8SC3, cv::Scalar(255, 255, 255));
		vc1 >> org;
		...
		cv::remap(org, dst, map_x, map_y, cv::INTER_LINEAR, cv::BORDER_CONSTANT, Scalar(0, 0, 0));
		...
		//	{
		//		
		//		v2d point1 = each_frame_destin[t].GetVertex(h, w);
		//		Point a(point1[0], point1[1]);
		//		circle(dst, a, 2, Scalar(255, 0, 0), -1);
		//	}
			...
			for (int w = 0; w < each_frame_destin[t].GetMeshWidth(); w++)
			{
				for (int x = -1; x <= 1; x++)
				{
					...
					line(dst, Point(each_frame_destin[t].GetVertex(h, w)[0], each_frame_destin[t].GetVertex(h, w)[1]),
						Point(each_frame_destin[t].GetVertex(h, w + x)[0], each_frame_destin[t].GetVertex(h, w + x)[1]), Scalar(255, 0, 0), 2);
				}
				for (int y = -1; y <= 1; y++)
				{
					...
					line(dst, Point(each_frame_destin[t].GetVertex(h, w)[0], each_frame_destin[t].GetVertex(h, w)[1]),
						Point(each_frame_destin[t].GetVertex(h + y, w)[0], each_frame_destin[t].GetVertex(h + y, w)[1]), Scalar(255, 0, 0), 2);
				}
			}
	...
	//显示原图
	//namedWindow("org");
	//imshow("org", org_single);

	//cv::remap(org_single, dst_single, map_x_single, map_y_single, cv::INTER_LINEAR);

	//namedWindow("dst");
	//imshow("dst", dst_single);
	//waitKey(0);
}
...
void union_smooth::CutSave()
{

	cv::Rect rect(50, 40, 300, 500);
	VideoCapture  read("D:/data/实验视频结果/Smooth-video14.mp4");
	//VideoCapture read("D:/data/实验视频/video14.mp4");
	for (int t = 0; t < frame_num; t++)
	{
		Mat frame;
		read >> frame;
		namedWindow("window");
		imshow("window", frame(rect));
		waitKey(0);
	}
