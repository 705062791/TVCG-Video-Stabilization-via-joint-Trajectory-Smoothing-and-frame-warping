...
%                             asap_a = AsSimilarAsPossibleWarping(obj.videoHeight, obj.videoWidth, obj.quadWidth, obj.quadHeight, 1);% * obj.CPthreshold(frameIndex));
%                             asap_a.SetControlPts(PAa, PBa);
%                             asap_a.Solve();     
%                             PaStitch(frameIndex, :, :, :, :) = asap_a.CalcHomos();
                            
                            PaStitch(frameIndex, :, :, :, :) = NewWarping(PAa, PBa, obj.videoHeight, obj.videoWidth, obj.quadHeight, obj.quadWidth, asaplambda);
%                             diff = sum(sum(sum(sum(test1 - test2))));
%                             if diff ~= 0
%                                 error('?'); 
%                             end
                            
                            
%                             asap_b = AsSimilarAsPossibleWarping(obj.videoHeight, obj.videoWidth, obj.quadWidth, obj.quadHeight, 1);% * obj.CPthreshold(frameIndex));
%                             asap_b.SetControlPts(PBb, PAb);
%                             asap_b.Solve();            
%                             PbStitch(frameIndex, :, :, :, :) = asap_b.CalcHomos();
                            PbStitch(frameIndex, :, :, :, :) = NewWarping(PBb, PAb, obj.videoHeight, obj.videoWidth, obj.quadHeight, obj.quadWidth, asaplambda);
%                         end
                        if CPcount ~= obj.ppf(frameIndex)
                            error('?');
                        end
                    end
                    toc; 
                    ...
                    for frameIndex = 1:obj.nFrames
                        ...
                        for row = 1:obj.meshSize
                            for col = 1:obj.meshSize
                                ...
                                nn = tail - head + 1;
                                % Pat - Cat
                                value = squeeze(obj.Ca(frameIndex, row, col, :, :)) * (crop_fix + 1);                            
                                % Pat - Par
%                                 if frameIndex > obj.span && frameIndex <= obj.nFrames - obj.span
...
%                                 end
                                ...
                                if ite > firstround && obj.nCPgrid(frameIndex, row, col, 1) > 10
                                    value = value + squeeze(PaStitch(frameIndex, row, col, :, :)) * squeeze(obj.Ca(frameIndex, row, col, :, :)) * stitching * 5;
                                    obj.Pa(frameIndex, row, col, :, :) = value / (crop_fix + obj.gamma_a(frameIndex, row, col) + stitching * 5);
                                end
                                
                                if ite > firstround && obj.nCPgrid(frameIndex, row, col, 1) <= 10
                                    value = value + squeeze(PaStitch(frameIndex, row, col, :, :)) * squeeze(obj.Ca(frameIndex, row, col, :, :)) * stitching * 0.5;
                                    obj.Pa(frameIndex, row, col, :, :) = value / (crop_fix + obj.gamma_a(frameIndex, row, col) + stitching * 0.5);
                                end

                                % Pb
                                value = squeeze(obj.Cb(frameIndex, row, col, :, :)) * (crop_fix + 1);
%                                 if frameIndex > obj.span && frameIndex <= obj.nFrames - obj.span
...
%                                 end
                                ...
                                if ite > firstround && obj.nCPgrid(frameIndex, row, col, 2) > 10
                                    value = value + squeeze(PbStitch(frameIndex, row, col, :, :)) * squeeze(obj.Cb(frameIndex, row, col, :, :)) * stitching * 5;                                    
                                    obj.Pb(frameIndex, row, col, :, :) = value / (crop_fix + obj.gamma_b(frameIndex, row, col) + stitching * 5);
                                end
                                if ite > firstround && obj.nCPgrid(frameIndex, row, col, 2) <= 10
                                    value = value + squeeze(PbStitch(frameIndex, row, col, :, :)) * squeeze(obj.Cb(frameIndex, row, col, :, :)) * stitching * 0.5;                                    
                                    obj.Pb(frameIndex, row, col, :, :) = value / (crop_fix + obj.gamma_b(frameIndex, row, col) + stitching * 0.5);
                                end
...
%             index = floor((PA(1, :) - 0.001) / qW) * 10 + ((floor(PB(2, :) - 0.001) / qH)) + 1;
%             if (max(index) > 100) || min(index) < 1
%                 error('?'); 
%             end
            obj.H = ransacfithomography(PA, PB, 0.001);
            %obj.H = EstimateH(PA', PB');
        ...
        function updateCP(obj)
            ...
            for frameIndex = 1:obj.nFrames
                ...
                dsort = sort(d(d ~= 0));
                threshold = mean(dsort(floor(length(dsort) * 4 / 5):floor(length(dsort) * 19/ 20))) * 3;
%                 threshold = max(threshold, 0.005 * obj.videoWidth);
            ...
            if sum(sum(sum(obj.nCPgrid(:, :, :, 1)))) ~= sum(sum(sum(obj.nCPgrid(:, :, :, 2))))
                error('?') ;
            end
            if sum(sum(sum(obj.nCPgrid(:, :, :, 1)))) ~= obj.nCP
               error('?') ;
            end 
        end
...
%                 end
        ...
        function renderGrid(obj, APath, BPath, outPath, gap)
            if ~exist(outPath, 'dir')
                mkdir(outPath);
            end
            obj.gap = gap;
            ...
            for frameIndex = 1:obj.nFrames %parfor
                ...
                if obj.useImage
                    ...
                    fileNameA = fileListA(frameIndex).name;
                    fileNameB = fileListB(frameIndex).name;
                    IA = imread([APath fileNameA]);
                    IB = imread([BPath fileNameB]);
                else
                    IA = readFrame(videoA);
                    IB = readFrame(videoB);
                end
        ...
        function imwarp = render1(obj, ab, I, frameIndex, P, C)
            ...
            for row = 1:obj.meshSize
                for col = 1:obj.meshSize
                    ...
                    if frameIndex > padding && frameIndex <= padding * 2
                        padOffset(row, col, :, :) = squeeze(obj.Pa(frameIndex - padding, row, col, :, :)) / squeeze(obj.Pa(padding, row, col, :, :));
                    end
                    if frameIndex > obj.nFrames - padding * 2 && frameIndex <= obj.nFrames - padding
                        padOffset(row, col, :, :) = squeeze(obj.Pa(frameIndex + padding, row, col, :, :)) / squeeze(obj.Pa(obj.nFrames - padding, row, col, :, :));
                    end
                end
            end
            ...
            for i = 0 : obj.meshSize
                for j = 0 : obj.meshSize
                    ...
                    y = j * obj.quadWidth + 1;
                    if i == 0 && j == 0
                        B11 = squeeze(padOffset(i+1, j+1, :, :)) * HH * squeeze(P(frameIndex, i+1, j+1, :, :)) ...
            / squeeze(C(frameIndex, i+1, j+1, :, :));
                    ...
                    if i == 0 && j == obj.meshSize
                        B10 = squeeze(padOffset(i+1, j, :, :)) * HH * squeeze(P(frameIndex, i+1, j, :, :))...
            ...
            / squeeze(C(frameIndex, i+1, j, :, :));
                    ...
                    if i>0 && i < obj.meshSize && j == 0
                        B11 = squeeze(padOffset(i+1, j+1, :, :)) * HH * squeeze(P(frameIndex, i+1, j+1, :, :))  ...
            / squeeze(C(frameIndex, i+1, j+1, :, :));
                        B01 = squeeze(padOffset(i, j+1, :, :)) * HH * squeeze(P(frameIndex, i, j+1, :, :))  ...
            ...
            / squeeze(C(frameIndex, i+1, j+1, :, :));
                        B01 = squeeze(padOffset(i, j+1, :, :)) * HH * squeeze(P(frameIndex, i, j+1, :, :))  ...
            / squeeze(C(frameIndex, i, j+1, :, :));
                        B00 = squeeze(padOffset(i, j, :, :)) * HH * squeeze(P(frameIndex, i, j, :, :))  ...
